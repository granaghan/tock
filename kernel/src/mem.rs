...
impl ReadWriteAppSlice {
    ...
    /// up to `ptr + len`, and no other slice in scope anywhere in the
    ...
    ///
    /// If the length is not `0`, the memory region of `ptr` up to
    /// `ptr + len` must be valid memory of the process of the given
